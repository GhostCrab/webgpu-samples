...
@group(0) @binding( 4) var<storage, read>       binIn: array<u32>;
@group(0) @binding( 5) var<storage, read_write> binOut: array<u32>;
@group(0) @binding( 6) var<storage, read>       binSumIn: array<u32>;
@group(0) @binding( 7) var<storage, read_write> binSumOut: array<u32>;
@group(0) @binding( 8) var<storage, read>       binPrefixSumIn: array<u32>;
@group(0) @binding( 9) var<storage, read_write> binPrefixSumOut: array<u32>;
@group(0) @binding(10) var<storage, read>       binIndexTrackerIn: array<u32>;
@group(0) @binding(11) var<storage, read_write> binIndexTrackerOut: array<u32>;
...
fn oneToTwo(index: u32, gridWidth: u32) -> vec2<u32> {
  var row = index / gridWidth;
  var col = index % gridWidth;
  return vec2(row, col);
}

fn twoToOne(vec2<u32> index, u32 gridWidth) -> u32 {
  var row = index.y;
  var col = index.x;
...
fn main(@builtin(global_invocation_id) GlobalInvocationID : vec3<u32>) {
  ...
  // load back
  {
    ...
    binOut[index] = u32(pos.x / binParams.count) + (u32(pos.y / binParams.count) * binParams.x);
  }
}
...
fn binSum(@builtin(global_invocation_id) GlobalInvocationID : vec3<u32>) {
  ...
  if (index < arrayLength(&verletObjectsIn)) {
    atomicAdd(binSumOut[binIn[index]], 1);
  }
}
...
fn binPrefixSum(@builtin(global_invocation_id) GlobalInvocationID : vec3<u32>) {
  ...
  for (var i = 0; i <= index; i++) {
    binPrefixSumOut[index] += binSumOut[i];
  }

  workgroupBarrier();

  binIndexTrackerOut[index] = 0;
  if (index > 0) {
    binIndexTrackerOut[index] = binPrefixSumOut[index - 1];
  }
}
